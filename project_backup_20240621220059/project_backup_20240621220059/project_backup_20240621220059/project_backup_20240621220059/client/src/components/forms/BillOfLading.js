...
const BillOfLading = ({ data, onChange, aiSuggestions }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      const response = await fetch('/api/forms/billoflading', {
        method: 'POST',
        headers: { 
      ...
      if (response.ok) {
        ...
        alert('Bill Of Lading submitted successfully!');
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      {/* Add form fields specific to BillOfLading here */}
      <input
        name="exampleField"
        ref={register}
