...
const PackingList = ({ data, onChange, aiSuggestions }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      const response = await fetch('/api/forms/packinglist', {
        method: 'POST',
        headers: { 
      ...
      if (response.ok) {
        ...
        alert('Packing List submitted successfully!');
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      {/* Add form fields specific to PackingList here */}
      <input
        name="exampleField"
        ref={register}
