...
const InsuranceCertificate = ({ data, onChange, aiSuggestions }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      const response = await fetch('/api/forms/insurancecertificate', {
        method: 'POST',
        headers: { 
      ...
      if (response.ok) {
        ...
        alert('Insurance Certificate submitted successfully!');
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      {/* Add form fields specific to InsuranceCertificate here */}
      <input
        name="exampleField"
        ref={register}
