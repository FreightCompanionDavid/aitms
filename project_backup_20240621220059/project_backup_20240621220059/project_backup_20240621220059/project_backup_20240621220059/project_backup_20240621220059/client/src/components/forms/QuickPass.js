...
const QuickPass = ({ data, onChange, aiSuggestions }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      const response = await fetch('/api/forms/quickpass', {
        method: 'POST',
        headers: { 
      ...
      if (response.ok) {
        ...
        alert('Quick Pass submitted successfully!');
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      {/* Add form fields specific to QuickPass here */}
      <input
        name="exampleField"
        ref={register}
