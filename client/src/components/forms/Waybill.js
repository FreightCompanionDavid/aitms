...
const Waybill = ({ data, onChange, aiSuggestions }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      const response = await fetch('/api/forms/waybill', {
        method: 'POST',
        headers: { 
      ...
      if (response.ok) {
        ...
        alert('Waybill submitted successfully!');
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      {/* Add form fields specific to Waybill here */}
      <input
        name="exampleField"
        ref={register}
