...
const ExportImportLicense = ({ data, onChange, aiSuggestions }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      const response = await fetch('/api/forms/exportimportlicense', {
        method: 'POST',
        headers: { 
      ...
      if (response.ok) {
        ...
        alert('Export Import License submitted successfully!');
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      {/* Add form fields specific to ExportImportLicense here */}
      <input
        name="exampleField"
        ref={register}
...
export default ExportImportLicense;
