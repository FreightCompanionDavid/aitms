...
const CommercialInvoice = ({ data, onChange, aiSuggestions }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      const response = await fetch('/api/forms/commercialinvoice', {
        method: 'POST',
        headers: { 
      ...
      if (response.ok) {
        ...
        alert('Commercial Invoice submitted successfully!');
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      {/* Add form fields specific to CommercialInvoice here */}
      <input
        name="exampleField"
        ref={register}
