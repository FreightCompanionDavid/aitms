...
const CustomsDeclaration = ({ data, onChange, aiSuggestions }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      const response = await fetch('/api/forms/customsdeclaration', {
        method: 'POST',
        headers: { 
      ...
      if (response.ok) {
        ...
        alert('Customs Declaration submitted successfully!');
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      {/* Add form fields specific to CustomsDeclaration here */}
      <input
        name="exampleField"
        ref={register}
